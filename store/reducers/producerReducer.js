  ...
  switch (action.type) {
    case types.ADD_PRODUCER:
      ...
      producers.forEach((producer) => {
        ...
        const min_producer_purchase =
          producerInfo.type_sale.type == 'kilos'
            ? producerInfo.min_producer_purchase * 1000
            : producerInfo.min_producer_purchase

        const totalForProducer = producerItems.reduce(
          (acu, cur) =>
            acu +
            (producerInfo.type_sale.type == 'kilos'
              ? cur.weight * cur.qty
              : producerInfo.type_sale.type == 'monto'
              ? cur.price_package * cur.qty
              : producerInfo.type_sale.type == 'unidades'
              ? cur.sale_format * cur.qty
              : cur.qty),
          0
        )
        ...
        producersProducts.push({
          producerInfo,
          products: cart.filter((cartItem) => cartItem.producer.brand_name === producer),
          totalItems: totalForProducer,
          complete,
          remaining,
        })
      })
